    ...
    try:
        ...
        while has_more and url:
            ...
            for e in j.get('events', []):
                # We use the instance's series_id to group them under the parent Event
                ...
                # If this is not a generic series parent object, it has a date we should record
...
def get_event_capacities():
    ...
    try:
        ...
        while has_more and url:
            ...
            j = r.json()
            for e in j.get('events', []):
                # For all instances (which are what's returned in the live feed), grab the ticket availability directly
                if not e.get('is_series'):
